...
def lens_blur(img, radius=3, components=5, exposure_gamma=5):
    ...
    # task_out = [[] for _ in range(len(components))]

    # for idx, (component, component_params) in enumerate(zip(components, parameters)):
    #     for channel in range(img.shape[0]):
    #         weight_sum = filter_task(idx, channel, img[channel], component, component_params)
    #         task_out[idx].append((channel, weight_sum))
    task_out = defaultdict(list)

    for idx, (component, component_params) in enumerate(zip(components, parameters)):
        for channel in range(img.shape[0]):
            result = filter_task(idx, channel, img[channel], component, component_params)
            idx, channel, weight_sum = result
            task_out[idx].append((channel, weight_sum))
